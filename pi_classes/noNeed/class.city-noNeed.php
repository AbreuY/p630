...
class city extends AbstractDB
{
	...
	function getRecordList($select_field='',$whereCnd=''){
		...
		if($whereCnd!='')
		{
			$cnd="select ".$select_field." from ".parent::SUFFIX."city where ".$whereCnd;			
		}
		else
		{
		 $cnd="select ".$select_field." from ".parent::SUFFIX."city where 1";
		}
	...
	{		
		$insert_sql="INSERT INTO `".parent::SUFFIX."city` ( ".$fields." ) VALUES (".$values.")";
	...
	{
		$this->result=$this->query("select * from ".parent::SUFFIX."city where `country_id`='".$rec_id."'");
		$rs=$this->getRow();
		return $rs;
	}//End Function
