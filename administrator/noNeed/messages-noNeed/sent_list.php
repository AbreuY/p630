<?php
	...
	if(isset($_REQUEST['btnDeleteAll']))
	{
		for($i=0; $i<$_REQUEST['totalcount']; $i++)
		{
			...
			{	
				$objDelEmp->deleteSentMessageToCust($_REQUEST['msg_id'.($i+1)]);
				$deleted = true;	
			}
		}
		if($deleted){
...
<body>
	...
	<section id="main" class="column">
		...
		<?php
			...
			$objSentMsgDtl->getSentMsgtoCustomer();
			$targetpage = "sent_list.php"; 	
			$limit = 15;
		...
		?>
		<header>
			<h3 class="tabs_involved">Message Sent to customer</h3>
		</header>
		<header>
		...
		<div class="tab_container">
			...
			<thead> 
				<tr> 
    				...
    				<th>To <a href="<?php echo AbstractDB::SITE_PATH;?>admin/messages/sent_list.php?page=<?php echo $_GET['page'];?>&sort=user.first_name&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/messages/sent_list.php?page=<?php echo $_GET['page'];?>&sort=user.first_name&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th>
					<th>Subject</th>
									
    				<th>Sent<a href="<?php echo AbstractDB::SITE_PATH;?>admin/messages/sent_list.php?page=<?php echo $_GET['page'];?>&sort=msg.msg_date&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/messages/sent_list.php?page=<?php echo $_GET['page'];?>&sort=msg.msg_date&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th> 
    				<th>Actions</th> 
				</tr> 
			</thead> 
			<tbody> 
					...
					<?php
						$objSentMsgDtl->getSentMsgtoCustomerPage($start, $limit,$_REQUEST['sort'],$_REQUEST['type']);
					$i=1;
					$slno=1;
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
    				...
    				<td><?php echo $objSentMsgDtl->getField('first_name')." "; echo $objSentMsgDtl->getField('last_name');?></td>
					<td><?php echo $objSentMsgDtl->getField('subject');?></td>
    				<td><?php echo date("jS F Y",strtotime($objSentMsgDtl->getField('msg_date')));?> </td> 
					<td>
						<a href="<?php echo ABstractDB::SITE_PATH;?>admin/messages/message_detail.php?type=sent&user=customer&message_id=<?php echo base64_encode($objSentMsgDtl->getField('message_id'));?>" title="Read"><img src="../images/icn_edit.png" border="0" /></a>
					</td> 
				</tr>
				<?php
					$i++;
