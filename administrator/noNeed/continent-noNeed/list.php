<?php
	...
	$objAddContinent=new Admin();	
	$objDelEmp=clone $objAddContinent;
	if($_POST['btnSubmit']=='Submit'){
		$objAddContinent=$objAddContinent->addContinentDetails($_POST);
	}
	...
	if(isset($_REQUEST['btnDeleteAll']))
	{
		for($i=0; $i<$_REQUEST['totalcount']; $i++)
		{
			...
			{	
				$objDelEmp->deleteContinentDetails($_REQUEST['emp_id'.($i+1)]);
				$deleted = true;	
			}
		}
		if($deleted){
...
function change_status(emp_id,status){
	...
	jQuery.ajax({
		...
		success:function(msg){
			window.location.href='<?php echo AbstractDB::SITE_PATH;?>admin/activity/list.php?page=<?php echo $_GET['page'];?>';
			Hide_Load();
		}
	})
}
...
<body>
	...
	<section id="main" class="column">
		...
		<?php
			$objContinentDtl=new Admin();
			$objContinentDtl->getAllcontinentDtl();
			$targetpage = "list.php"; 	
			$limit = 15;
			$total_pages = $objContinentDtl->numofrows();	
			$stages = 12;
		...
		?>
		<header>
			<h3 class="tabs_involved">Continent Management </h3>
		</header>
		<header>
		...
		<div class="tab_container">
			...
			<thead> 
				<tr> 
    				...
    				<th>Continent ID <a href="<?php echo AbstractDB::SITE_PATH;?>admin/continent/list.php?page=<?php echo $_GET['page'];?>&sort=continent_id&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/continent/list.php?page=<?php echo $_GET['page'];?>&sort=continent_id&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th>
					<th>Continent Name <a href="<?php echo AbstractDB::SITE_PATH;?>admin/continent/list.php?page=<?php echo $_GET['page'];?>&sort=continent_name &type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/continent/list.php?page=<?php echo $_GET['page'];?>&sort=continent_name&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th>
					<th> Action </th>
				</tr> 
			</thead> 
			<tbody> 
					...
					<?php
					$objContinentDtl->getAllContinentDtlPage($start, $limit,$_REQUEST['sort'],$_REQUEST['type']);
					$i=1;
					$slno=1;
					while($objContinentDtl->getAllRow()){
						if($i%2){
							$bg_color="#E0E0E3";
						}else{
							$bg_color="#FFFFFF";
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						...
						<input type="hidden" name="emp_id" id="emp_id" value="<?php echo $objContinentDtl->getField('continent_id');?>" />
						<input type="hidden" name="emp_id<?=$slno?>" id="emp_id<?=$slno?>" value="<?php echo $objContinentDtl->getField('continent_id');?>" />
					</td> 
    				<td><?php echo $objContinentDtl->getField('continent_id');?></td>
					<td><?php echo $objContinentDtl->getField('continent_name');?></td>
    				<td>
						<a href="add.php?act_id=<?php echo $objContinentDtl->getField('continent_id');?>" title="Edit"><img src="../images/icn_edit.png" border="0" /></a>
					</td> 
				</tr>
				<?php
					$i++;
