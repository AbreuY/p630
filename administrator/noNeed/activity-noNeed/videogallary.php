<?php
	...
	//$objAddVideoGallaryBody=new Admin();
	
	$objAddVideoGallary=new Admin();

	/*
	if($_POST['btnSubmit']=='Submit'){
		$objAddvideogallaryBody->addEmployeeDetails($_POST);
	}*/
	...
	if(isset($_REQUEST['btnDeleteAll']))
	{
		for($i=0; $i<$_REQUEST['totalcount']; $i++)
		{
			...
			{	
				$objDelvideogallary=clone $objAddVideoGallary;
				$objDelvideogallary->deleteVedioGallaryVdoDetails($_REQUEST['id'.($i+1)]);
				$deleted = true;	
			}
		}
		if($deleted){
...
/*function change_status(emp_id,status){
	...
	jQuery.ajax({
		...
		success:function(msg){
			window.location.href='<?php echo AbstractDB::SITE_PATH;?>admin/videogallary.php?page=<?php echo $_GET['page'];?>';
			Hide_Load();
		}
	})
}*/
...
<body>
	...
	<section id="main" class="column">
		...
		<div id="loading"></div>
		<form name="frmVideoGallary" id="frmvideogallary" method="post" action="">
		<?php
		 
			$objvideogallaryDtl=new Admin();
			
			$objvideogallaryDtl->getVideoGallaryVdoDetails($activity_id);
			$targetpage = "videogallary.php"; 	
			$param="&act_id=".$_GET['act_id'];
			$limit = 2;
			$total_pages = $objvideogallaryDtl->numofrows();	
			$stages = 2;
		...
		<header>
				<?php
					$objGetActName=clone $objvideogallaryDtl;
					$objGetActName->getActivityNameById($_REQUEST['act_id']);
					$objGetActName->getAllRow();
				?>
			<h3 class="tabs_involved">Activity Video Gallery Management for <font color="#0000FF"><?php echo $objGetActName->getField('activity_booker_name');?></font> </h3>
			
		</header>
		<header>
			<div style="float: left;padding: 5px;">
				<a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/list.php?user_id=<?php echo $user_id?>" title="Back"> <img src="../images/back_icon2.png" border="0" /></a>
				&nbsp;&nbsp;
				<input type="submit" name="btnDeleteAll" id="btnDeleteAll" value="Delete All" class="alt_btn" onclick="return delconfirm();" />
				&nbsp;&nbsp;
				<a href="upload_video_gallary_vdo.php?act_id=<?php echo base64_encode($activity_id);?>&user_id=<?php echo $user_id;?>" title="Upload new Video"> <img src="<?php echo AbstractDB::SITE_PATH;?>images/add_video.jpg" border="0" /> Add new video </a>
				
				<a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/viewholiday.php?user_id=<?php echo $_REQUEST['user_id'];?>&act_id=<?php echo $_REQUEST['act_id'];?>" title="Add activity holiday"><img src="<?php echo AbstractDB::SITE_PATH;?>images/holiday_header_icon.png" alt="Add activity holiday" title="Add activity holiday" />Next</a>
			</div>
		</header>
		<div class="tab_container">
			...
			<thead> 
				<tr> 
    				...
    				<th>Video Title <a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/videogallary.php?page=<?php echo $_GET['page'];?>&sort=video_name&type=DESC&act_id=<?php echo $_REQUEST['act_id'];?>&user_id=<?php echo $user_id;?>"><img src="<?php echo AbstractDB::SITE_PATH;?>admin/images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/videogallary.php?page=<?php echo $_GET['page'];?>&sort=video_name&type=ASC&act_id=<?php echo $_REQUEST['act_id'];?>&user_id=<?php echo $user_id;?>"><img src="<?php echo AbstractDB::SITE_PATH;?>admin/images/sort_down.gif" border="0" /></a></th>
					<th>Video URL</th>
					<th>Actions</th> 
				</tr> 
			</thead> 
			<tbody> 
					...
					<?php
					$objvideogallaryDtl->getAllvideogallaryImgDtlPage($activity_id,$start, $limit,$_REQUEST['sort'],$_REQUEST['type']);
					$i=1;
					$slno=1;
					//print_r($objvideogallaryDtl);die();
					while($objvideogallaryDtl->getAllRow()){
						if($i%2){
							$bg_color="#E0E0E3";
						}else{
							$bg_color="#FFFFFF";
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						...
						<input type="hidden" name="id" id="id" value="<?php echo base64_encode($objvideogallaryDtl->getField('id'));?>" />
						<input type="hidden" name="id<?=$slno?>" id="id<?=$slno?>" value="<?php echo base64_encode($objvideogallaryDtl->getField('id'));?>" />
					</td> 
    				<td><?php echo stripslashes($objvideogallaryDtl->getField('video_name'));?></td>					
					<td>
											<div class="feaild_otr">
														<div class="meadia_section" style="width:300;">						
																	<div class="part" style="width:200px;">
																		<div class="part_main_otr">
																			<div class="part_main">													
																				<div class="media">
																				<?php 
																					if($objvideogallaryDtl->getField('video_url')!=''){
																								echo stripslashes($objvideogallaryDtl->getField('video_url')) ;
																					}
																					else{ 
					...
					<td>
						<a href="edit_upload_video_gallary_vdo.php?vdo_id=<?php echo base64_encode($objvideogallaryDtl->getField('id'));?>&act_id=<?php echo $_REQUEST['act_id'];?>&user_id=<?php echo $_REQUEST['user_id'];?>" title="Edit"><img src="../images/icn_edit.png" border="0" /> </a>						
					</td> 
				</tr>
				<?php
					$i++;
