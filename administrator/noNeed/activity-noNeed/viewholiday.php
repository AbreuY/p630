<?php
	...
	if(isset($_REQUEST['btnDeleteAll']))
	{
		for($i=0; $i<$_REQUEST['totalcount']; $i++)
		{
			...
			{	
				$objDelEmp->deleteActivityHoliday($_REQUEST['emp_id'.($i+1)]);
				$deleted = true;	
			}
		}
		if($deleted){
...
<body>
	...
	<section id="main" class="column">
		...
		<?php
			$objActivityHolidayDtl=new Admin();
			$objActivityHolidayDtl->getActivityHolidayDtlById($_REQUEST['act_id']);
			$objActivityHolidayDtl->getAllRow();
			$targetpage = "viewholiday.php"; 	
			$limit = 5;
			$total_pages = $objActivityHolidayDtl->numofrows();	
			$stages = 2;
		...
		<header>
			<h3 class="tabs_involved">Activity Holiday For
			<font color="#0000FF"><?php 
					$objActivityName=Clone $objActivityHolidayDtl;
		...
		<header>
			<div style="float: left;padding: 5px;">
				...
				<a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/addholiday.php?user_id=<?php echo $_REQUEST['user_id'];?>&act_id=<?php echo $_REQUEST['act_id'];?>" title="View holiday details"><img src="<?php echo AbstractDB::SITE_PATH;?>images/holiday_header_icon.png" title="Update holiday" />Add Holiday</a>
			</div>
		</header>
		<div class="tab_container">
			...
			<thead> 
				<tr> 
    				...
    				<th>Holiday<a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/viewholiday.php?user_id=<?php echo $_REQUEST['user_id'];?>&act_id=<?php echo $_REQUEST['act_id'];?>&page=<?php echo $_GET['page'];?>&sort=holiday&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/viewholiday.php?user_id=<?php echo $_REQUEST['user_id'];?>&act_id=<?php echo $_REQUEST['act_id'];?>&page=<?php echo $_GET['page'];?>&sort=holiday&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th>
					<th>Type<a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/viewholiday.php?user_id=<?php echo $_REQUEST['user_id'];?>&act_id=<?php echo $_REQUEST['act_id'];?>&page=<?php echo $_GET['page'];?>&sort=day_recur&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/activity/viewholiday.php?user_id=<?php echo $_REQUEST['user_id'];?>&act_id=<?php echo $_REQUEST['act_id'];?>&page=<?php echo $_GET['page'];?>&sort=day_recur&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th>
			...
			<tbody> 
					...
					<?php
						...
						$objActivityHolidayDtl->getActivityHolidayDtlPage($start, $limit,$_REQUEST['sort'],$_REQUEST['type'],$_REQUEST['act_id']);
					$i=1;
					$slno=1;
					while($objActivityHolidayDtl->getAllRow()){
						if($i%2){
							$bg_color="#E0E0E3";
						}else{
							$bg_color="#FFFFFF";
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						...
						<input type="hidden" name="emp_id" id="emp_id" value="<?php echo $objActivityHolidayDtl->getField('id');?>" />
						<input type="hidden" name="emp_id<?=$slno?>" id="emp_id<?=$slno?>" value="<?php echo $objActivityHolidayDtl->getField('id');?>" />
					</td> 
    				<td><?php echo $objActivityHolidayDtl->getField('holiday');?></td>
    				<td><?php $day_type=$objActivityHolidayDtl->getField('day_recur');
						if($day_type=="D")						{
							echo "Day";
						}elseif($day_type=="W"){
							echo "Weekly";
					...
					?></td>
					<td><?php echo $objActivityHolidayDtl->getField('reccursive');?></td>
					<td><a href="<?php echo ABstractDB::SITE_PATH;?>admin/activity/edit_holiday.php?user_id=<?php echo $_REQUEST['user_id'];?>&act_id=<?php echo base64_encode($objActivityHolidayDtl->getField('activity_id'));?>&holiday_id=<?php echo base64_encode($objActivityHolidayDtl->getField('id'));?>" title="Edit"><img src="../images/icn_edit.png" border="0" /></a>
					</td>  
