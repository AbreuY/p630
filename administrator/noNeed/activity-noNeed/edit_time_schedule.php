<?php
	...
	if($_POST['btnSubmit']=='Update'){
		//print_r($_POST);die();
	$objAddTimeSchedule->UpdateActTimeShcedule($_POST);
...
<body>
	...
	<section id="main" class="column">		
		...
		<?php include("../activity_menu.php")?>
			...
			<header><h3>Edit Activity Time Schedule
			<font color="#0000FF"><?php 
				...
				<?php 
					$objActivityTimeSchedule=clone $objActivityName;
					$objActivityTimeSchedule->getActivityTimeScheduleById($_REQUEST['time_schedule_id']);
				...
				<div class="module_content">
						...
						<fieldset style="width:48%; float:left; margin-right: 3%;">
							 <label for="time_hour">Select Hour<sup style="color:#FF0000;font-weight:bold;">*</sup></label>
								...
								<input type="hidden" name="time_schedule_id" id="time_schedule_id" value="<?php echo $objActivityTimeSchedule->getField('time_schedule_id');?>" />
									<select name="time_hour" id="time_hour" class="validate[required] text-input">
										...
										{
										?>
										<option value="<?php if(strlen($hour)==1){ echo "0".$hour;} else {echo $hour;}?>"<?php if($objActivityTimeSchedule->getField('hour')==$hour){?> selected="selected" <?php }?>><?php if(strlen($hour)==1){echo "0".$hour;} else { echo $hour;}?></option>
										<?php
										}
									?>	
									</select>   
						...
						<fieldset style="width:48%; float:left; margin-right: 3%;">
							  <label for="day_type">Select Minute<sup style="color:#FF0000;font-weight:bold;">*</sup></label>
									<select name="time_minute" id="time_minute" class="validate[required] text-input">
										...
										{
										?>
										<option value="<?php if(strlen($minute)==1){echo "0".$minute;} else { echo $minute;}?>" <?php if($objActivityTimeSchedule->getField('minute')==$minute){?> selected="selected" <?php }?>><?php if(strlen($minute)==1){echo "0".$minute;} else { echo $minute;}?></option>
										<?php 
										}
										?>
									</select>   
						...
						<fieldset style="width:48%; float:left;">
							<label for="time_minute">Select Schedule At<sup style="color:#FF0000;font-weight:bold;">*</sup></label>
									<select name="schedule_at" id="schedule_at">
												<option value="am" <?php if($objActivityTimeSchedule->getField('schedule_at')=='am'){?> selected="selected" <?php }?>>am</option>						
												<option value="pm" <?php if($objActivityTimeSchedule->getField('schedule_at')=='pm'){?> selected="selected" <?php }?>>pm</option>
									</select>  
						</fieldset><div class="clear"></div>	
					</div>
			<footer>
