<?php
	...
	if($_REQUEST['action']=='delete'){
		$cnf=$objDelEmp->deleteEmployeeDetails($_REQUEST['admin_id']);
		if($cnf){
	...
	if(isset($_REQUEST['btnDeleteAll']))
	{
		for($i=0; $i<$_REQUEST['totalcount']; $i++)
		{
			...
			{	
				$objDelEmp->deleteEmployeeDetails($_REQUEST['admin_id'.($i+1)]);
				$deleted = true;	
			}
		}
		if($deleted){
...
function change_status(admin_id,status){
	...
	jQuery.ajax({
		...
		type: "post",
		data:"action=chnage_emp_status&admin_id="+admin_id+"&status="+status,
		success:function(msg){
			window.location.href='<?php echo AbstractDB::SITE_PATH;?>administrator/admin_management/manage_employee_details.php?page=<?php echo $_GET['page'];?>';
			Hide_Load();
		}
	})
}
...
<body>
	...
	<section id="main" class="column">
		...
		?>
		<header>
			<h3 class="tabs_involved">Admin Management </h3>
		</header>
		<header>
		...
		<div class="tab_container">
			...
			<thead> 
				<tr> 
    				...
    				<th>First Name <a href="<?php echo AbstractDB::SITE_PATH;?>administrator/admin_management/manage_employee_details.php?page=<?php echo $_GET['page'];?>&sort=first_name&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>administrator/admin_management/manage_employee_details.php?page=<?php echo $_GET['page'];?>&sort=first_name&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th>
    				...
    				<th>Created On <a href="<?php echo AbstractDB::SITE_PATH;?>administrator/admin_management/manage_employee_details.php?page=<?php echo $_GET['page'];?>&sort=add_date&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>administrator/admin_management/manage_employee_details.php?page=<?php echo $_GET['page'];?>&sort=add_date&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th> 
    				<th>Status</th> 
					<th>Actions</th> 
				</tr> 
			</thead> 
			<tbody> 
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						...
						<input type="hidden" name="admin_id" id="admin_id" value="<?php echo base64_encode($objEmpDtl->getField('admin_id'));?>" />
						<input type="hidden" name="admin_id<?=$slno?>" id="admin_id<?=$slno?>" value="<?php echo base64_encode($objEmpDtl->getField('admin_id'));?>" />
					</td> 
					...
					<td><a href="javascript:void(0);" onclick="change_status('<?php echo base64_encode($objEmpDtl->getField('admin_id'));?>','<?php echo $objEmpDtl->getField('active');?>');"> <?php if($objEmpDtl->getField('active')){ echo "Active";}else{ echo "Inactive";}?></a></td> 
    				<td>
						<a href="edit_employee.php?admin_id=<?php echo base64_encode($objEmpDtl->getField('admin_id'));?>" title="Edit"><img src="../images/icn_edit.png"  border="0" /></a>
					</td> 
				</tr>
				<?php
					$i++;
