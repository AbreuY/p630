...
<body>
	...
	<section id="main" class="column">
		...
		<?php
			...
			$targetpage = "list.php"; 	
			$limit = 15;
			$total_pages = $objCategoryDtl->numofrows();	
			$stages = 12;
		...
		?>
		<header>
			<h3 class="tabs_involved">Category Management </h3>
		</header>
		<header>
		...
		<div class="tab_container">
			...
			<thead> 
				<tr> 
					...
					<th>Category Name <a href="<?php echo AbstractDB::SITE_PATH;?>administrator/category_management/list.php?page=<?php echo $_GET['page'];?>&sort=cat_name&type=DESC"><img src="../images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>administrator/category_management/list.php?page=<?php echo $_GET['page'];?>&sort=cat_name&type=ASC"><img src="../images/sort_down.gif" border="0" /></a></th>
					<th>&nbsp;  </th>
                    <th> Action </th>
				</tr> 
			</thead> 
			<tbody> 
					...
					<?php
					$objCategoryDtl->getAllCategoryDtlPage($start, $limit,$_REQUEST['sort'],$_REQUEST['type']);
					$i=1;
					$slno=1;
					while($objCategoryDtl->getAllRow()){
						if($i%2){
							$bg_color="#E0E0E3";
						}else{
							$bg_color="#FFFFFF";
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						...
						<input type="hidden" name="cat_id" id="cat_id" value="<?php echo $objCategoryDtl->getField('cat_id');?>" />
						<input type="hidden" name="cat_id<?=$slno?>" id="cat_id<?=$slno?>" value="<?php echo $objCategoryDtl->getField('cat_id');?>" />
					</td> 
					<td><?php echo $objCategoryDtl->getField('cat_name');?></td>
					<td><?php 
							if($objCategoryDtl->getField('parent_id')){
							#clone::getParentCatName:
							$objPrntCat = clone $obj_cat;
							$objPrntCat->getCategoryDtlById($objCategoryDtl->getField('parent_id'));
							$prntCatInfo = $objPrntCat->getAllRow();
							echo "<b>".$prntCatInfo['cat_name']."</b>";
							}else{
						?>
                        	<strong>Main Cat.</strong>	
                        <?php
							}
                        ?>
                    </td>                    
    				<td>
						<a style="font-family: Arial; font-size: 12px; border: 0px none;" href="add.php?edit_id=<?php echo base64_encode($objCategoryDtl->getField('cat_id')); ?>"><img src="../images/icn_edit.png"></a></td>
					</td> 
				</tr>
				<?php
					$i++;
