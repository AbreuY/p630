...
<body>
	...
	<section id="main" class="column">
		...
		#Objects:
			$objEdu = new Admin();
			#clone:
			$objEduGet = clone $objEdu;
			$objEduGet->getAllEdu();
			$targetpage = "list.php"; 	
			$limit = 15;
			$total_pages = $objEduGet->numofrows();	
			$stages = 12;
		...
		?>
		<header>
			<h3 class="tabs_involved">Educational Institute Management </h3>
		</header>
		<header>
		...
		<div class="tab_container">
			...
			<tbody> 
					...
					<?php
					$objEduGet->getAllEduPage($start, $limit,$_REQUEST['sort'],$_REQUEST['type']);
					$i=1;
					$slno=1;
					while($objEduGet->getAllRow()){
						if($i%2){
							$bg_color="#E0E0E3";
						}else{
							$bg_color="#FFFFFF";
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						<input type="checkbox" id="chk_<?=$slno?>" name="chk_<?=$slno?>" value="<?php echo  $objEduGet->getField('edu_id');?>" onclick="check_for_all(this)" />
					</td> 
					<td><?php echo $objEduGet->getField('name');?></td>                
    				<td>
						<a style="font-family: Arial; font-size: 12px; border: 0px none;" href="add.php?edit_id=<?php echo base64_encode($objEduGet->getField('edu_id')); ?>"><img src="../images/icn_edit.png"></a></td>
					</td> 
				</tr>
				<?php
					$i++;
