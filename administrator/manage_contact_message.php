<?php
	...
	session_start();
	chkAdminLogin();
	$objAddCntMessage=new Admin();	
	$objDelCntMessage=clone $objAddCntMessage;
	if(isset($_REQUEST['btnDeleteAll']))
	{
		for($i=0; $i<$_REQUEST['totalcount']; $i++)
		{
			...
			{	
				$objDelCntMessage->deleteCntMsgDetails($_REQUEST['cnt_msg_id'.($i+1)]);
				$deleted = true;	
			}
		}
		if($deleted){
...
<body>
	...
	<section id="main" class="column">
		...
		<?php
			$objCntMessageDtl=new Admin();
			$objCntMessageDtl->getAllCntPersonDtl();
			$targetpage = "manage_contact_message.php"; 	
			$limit = 15;
			$total_pages = $objCntMessageDtl->numofrows();	
			$stages = 12;
		...
		?>
		<header>
			<h3 class="tabs_involved">Contact Message Management </h3>
		</header>
		<header>
		...
		<div class="tab_container">
			...
			<thead> 
				<tr> 
    				...
    				<th>Name <a href="<?php echo AbstractDB::SITE_PATH;?>admin/manage_contact_message.php?page=<?php echo $_GET['page'];?>&sort=name&type=DESC"><img src="images/sort_up.gif" border="0" /></a><a href="<?php echo AbstractDB::SITE_PATH;?>admin/manage_contact_message.php?page=<?php echo $_GET['page'];?>&sort=name&type=ASC"><img src="images/sort_down.gif" border="0" /></a></th>
					<th>E-Mail </th>
					<th>Message </th>					
    				<th>Status</th> 
			</tr> 
			...
			<tbody> 
					...
					<?php
					$objCntMessageDtl->getAllCntMsgPage($start, $limit,$_REQUEST['sort'],$_REQUEST['type']);
					$i=1;
					$slno=1;
					while($objCntMessageDtl->getAllRow()){
						if($i%2){
							$bg_color="#E0E0E3";
						}else{
							$bg_color="#FFFFFF";
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						...
						<input type="hidden" name="cnt_msg_id" id="cnt_msg_id" value="<?php echo base64_encode($objCntMessageDtl->getField('id'));?>" />
						<input type="hidden" name="cnt_msg_id<?=$slno?>" id="cnt_msg_id<?=$slno?>" value="<?php echo base64_encode($objCntMessageDtl->getField('id'));?>" />
					</td> 
    				<td><?php echo $objCntMessageDtl->getField('name');?></td>
					<td><a href="mailto:<?php echo $objCntMessageDtl->getField('email');?>"><?php echo $objCntMessageDtl->getField('email');?></a></td>
    				<td><?php echo substr($objCntMessageDtl->getField('message'),0,50); if(strlen($objCntMessageDtl->getField('message'))>50){echo "<b> . . .</b>";}?></td> 
					 <td><a href="msg.php?id=<?php echo base64_encode($objCntMessageDtl->getField('id'));?>">
					 <?php if($objCntMessageDtl->getField('read_status')=='No') {?>UnRead<?php }else{?>Read<?php } ;?></a></td>
				</tr>
				<?php
					$i++;
					$slno++;
