...
<body>
	...
	<section id="main" class="column">
		...
		#Objects:
			$objEmp = new Admin();
			#clone:
			$objEmpGet = clone $objEmp;
			$objEmpGet->getAllEmp();
			$targetpage = "list.php"; 	
			$limit = 15;
			$total_pages = $objEmpGet->numofrows();	
			$stages = 12;
		...
		<div class="tab_container">
			...
			<tbody> 
					...
					<?php
					$objEmpGet->getAllEmpPage($start, $limit,$_REQUEST['sort'],$_REQUEST['type']);
					$i=1;
					$slno=1;
					while($objEmpGet->getAllRow()){
						if($i%2){
							$bg_color="#E0E0E3";
						}else{
							$bg_color="#FFFFFF";
				...
				<tr style='background-color:<?php echo $bg_color;?>;'> 
   					<td>
						<input type="checkbox" id="chk_<?=$slno?>" name="chk_<?=$slno?>" value="<?php echo  $objEmpGet->getField('emp_id');?>" onclick="check_for_all(this)" />
					</td> 
					<td><?php echo $objEmpGet->getField('name');?></td>                
    				<td>
						<a style="font-family: Arial; font-size: 12px; border: 0px none;" href="add.php?edit_id=<?php echo base64_encode($objEmpGet->getField('emp_id')); ?>"><img src="../images/icn_edit.png"></a></td>
					</td> 
				</tr>
				<?php
					$i++;
